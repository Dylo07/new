...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('admin.packages.update', $package) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow rounded-lg p-6">
                ...
                <div>
                    ...
                    <select name="type" 
                        ...
                        @foreach($packageTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('type', $package->type) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="flex">
                        <select name="currency" class="border border-gray-300 rounded-l-lg px-3 py-2 bg-gray-50">
                            <option value="LKR" {{ old('currency', $package->currency) == 'LKR' ? 'selected' : '' }}>LKR</option>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" 
                          id="description" 
                          rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $package->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            @if($package->image_path)
                <div class="mt-6">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <p class="text-sm text-gray-600">Current package image</p>
        ...
        <div id="pricing-tiers-section" class="bg-white shadow rounded-lg p-6" style="{{ $package->type === 'wedding' ? 'display: block;' : 'display: none;' }}">
            <h2 class="text-xl font-semibold text-gray-800 mb-6">Pricing Tiers (Wedding Packages)</h2>
            
            <div id="pricing-tiers-container">
                @php
                    $pricingTiers = old('pricing_tiers', $package->pricing_tiers ?? []);
                @endphp
                @if($pricingTiers && count($pricingTiers) > 0)
...
@push('scripts')
<script>
let featureIndex = {{ $features ? count($features) : 1 }};
let pricingTierIndex = {{ $pricingTiers ? count($pricingTiers) : 0 }};
